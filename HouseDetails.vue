	...
	methods: {
		...
		countDown(times){
			...
			timer=setInterval(()=>{
				...
				this.operation_btn_text = (day+"天:"+hour+"小时："+minute+"分钟："+second+"秒");
				times--;
				if(times <0){
					clearInterval(timer);
		...
		getApplyForFunc(){
			if(this.$store.getters.userInfo){
				...
				}else if(this.$store.getters.userInfo.State == 1007 && this.operation_btn_text == '可选房'){
		...
		//判断按钮的状态
		JudgeBtn(RoomState,FamilyType,HouseTypeName,SplitedSex,SplitedMark,systemDate,SecSpan,roomAddr,IsSelect,IsThreeCancel){//房间状态判断 | 用户可选户型 | 户型 | 合租性别 | 拆套房间 | 系统时间 |选房时间|看房地址|该房间是否曾被选过|该房间是否取消过三次
			//判断是否到的选房时间
				...
				getUserInfo({AccountId:this.$store.getters.userInfo.AccountId}).then(response => {
					...
					if(RoomState == 1){
						...
						if(response.Data.State == 0 || response.Data.State == ''){
							...
							}else if(IsSelect == 1 && IsThreeCancel == 0){
								this.operation_btn_text = '此房您已取消，两天内不能选取该房间'
								this.isDisabled =true
							}else if(IsSelect == 0 && IsThreeCancel == 1){
								this.operation_btn_text = '此房您已连续取消3次，不能选取该房间'
								this.isDisabled =true
							}else if(IsSelect == 1 && IsThreeCancel == 1){
								this.operation_btn_text = '此房间您已不能选取'
								this.isDisabled =true
							}else{
								this.operation_btn_text = '此房间您不能选取'
								this.isDisabled =true
							}
			...
			getHouseDetails(params).then(response => {
				...
				switch(response.StatusCode){
					case 200 :
						...
						this.FloorName=data.FloorName//楼层
						...
						//相似的房源
						//按钮状态的判断
						this.JudgeBtn(data.BaseInfo.State,data.FamilyType,data.BaseInfo.RoomTypeName,data.BaseInfo.SplitedSex,data.BaseInfo.SplitedMark,data.SystemTime,data.BaseInfo.SecSpan,data.BaseInfo.AllName,data.IsSelect,data.IsThreeCancel)//传如房间的状态
						setTimeout(()=>{
							this.loading =false
						},1000)
					break;
